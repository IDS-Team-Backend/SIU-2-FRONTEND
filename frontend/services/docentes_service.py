...
def obtener_equipo_docente(curso_id):
    ...
    ok, data = api_request("GET", "/curso_docentes/", params={"curso_id": curso_id})
    if not ok:
...
def agregar_docente(curso_id, docente_id, rol):
    ...
    ok, data = api_request("POST", "/curso_docentes/", json_body={
        "curso_id":   curso_id,
...
def cambiar_participacion(integrante_id, rol):
    ...
    ok, data = api_request("PATCH", f"/curso_docentes/{integrante_id}",
                           json_body={"nombre": rol})
    if not ok:
...
def quitar_docente(integrante_id):
    ok, data = api_request("DELETE", f"/curso_docentes/{integrante_id}")
    if not ok:
